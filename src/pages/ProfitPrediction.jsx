...
import { cropsList, profitPredictionResult } from '@/utils/mockData';
...
export default function ProfitPrediction() {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      setResult(profitPredictionResult);
    }, 1500);
  };
  ...
  const chartData = result ? [
    { name: 'Financials', Revenue: result.revenue, Cost: result.totalCost, Profit: result.profit }
  ] : [];
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" name="area" value={formData.area} onChange={handleChange} className="input-field w-full" required />
              </div>
              
              <div>
                <label className="block text-sm font-medium text-surface-700 dark:text-surface-200 mb-1">Expected Yield (tons/acre)</label>
                <input type="number" name="yield" value={formData.yield} onChange={handleChange} className="input-field w-full" required />
              </div>
              
              <div className="col-span-2">
                <label className="block text-sm font-medium text-surface-700 dark:text-surface-200 mb-1">Market Price (₹/quintal)</label>
                <input type="number" name="price" value={formData.price} onChange={handleChange} className="input-field w-full" required />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" name="fertilizer" value={formData.fertilizer} onChange={handleChange} className="input-field w-full" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-surface-700 dark:text-surface-200 mb-1">Labor Cost</label>
                <input type="number" name="labor" value={formData.labor} onChange={handleChange} className="input-field w-full" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-surface-700 dark:text-surface-200 mb-1">Irrigation Cost</label>
                <input type="number" name="irrigation" value={formData.irrigation} onChange={handleChange} className="input-field w-full" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-surface-700 dark:text-surface-200 mb-1">Other Costs</label>
                <input type="number" name="other" value={formData.other} onChange={handleChange} className="input-field w-full" required />
              </div>
            </div>
              ...
              <div className="card p-6 bg-gradient-to-br from-primary-50 to-primary-100 dark:from-primary-900/20 dark:to-primary-900/10 border-primary-200 dark:border-primary-800 border-2">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-4xl font-extrabold text-primary-700 dark:text-primary-400">{formatCurrency(result.profit)}</p>
                  </div>
